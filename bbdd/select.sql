...
select DISTINCT nombre from FUTBOLISTAS;
select DISTINCT posicion "Demarcaciones" from FUTBOLISTAS;
SELECT * from FUTBOLISTAS "MBAPES";
SELECT apellidos "Futbolistas" from FUTBOLISTAS F where posicion= 'DEFENSA'order by f.apellidos desc;
select DISTINCT nombre, apellidos,fecha_nacimiento  from FUTBOLISTAS order by 3;
SELECT DISTINCT apellidos from FUTBOLISTAS where posicion ='DEFENSA' order by apellidos desc;
select apellidos || ', ' || nombre "futbolistas" from futbolistas order by FUTBOLISTAS.POSICION;
SELECT * from equipos where presupuesto !=1000000;
SELECT * from equipos where presupuesto <>1000000;
SELECT * from equipos where presupuesto ^=1000000;
select nombre "Futbolistas",salario "Money" from FUTBOLISTAS where salario >150000;
select ID "CODIGO", apellidos "los gomez" from FUTBOLISTAS where apellidos ='GOMEZ';
...
SELECT nombre "futbolistas", salario "money" from FUTBOLISTAS where salario >=100000 and salario <= 200000 order by salario;
select nombre, apellidos, salario*1.1 from FUTBOLISTAS where posicion = 'PORTERO' order by FUTBOLISTAS.APELLIDOS;
